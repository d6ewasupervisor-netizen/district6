...
import { sendAdminPasswordResetEmail } from '../lib/email.js';
...
router.post('/complete-password-reset', resetCompleteLimiter, async (req, res) => {
  ...
  try {
    const rawTok =
      req.body && req.body.token != null ? String(req.body.token).trim() : '';
    if (rawTok.length < 24) {
...
router.post('/password', changePasswordLimiter, requireAdmin, async (req, res) => {
  try {
    ...
    if (!row?.password_hash) {
      return res.status(400).json({
        ok: false,
        error: 'Account is not ready for password changes.',
      });
    }

    const rawCur =
      req.body && req.body.currentPassword != null
    ...
    const rawConfirm =
      req.body && req.body.newPasswordConfirm != null
        ? String(req.body.newPasswordConfirm)
        : '';
    const a = normalizePassword(rawPw);
    const b = normalizePassword(rawConfirm);
